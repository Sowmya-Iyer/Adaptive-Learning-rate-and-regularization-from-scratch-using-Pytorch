...
def main(*ARGS):
    ...
    dataset_train = (
        torchvision.datasets.MNIST(
            root=source, train=True, download=True,
            transform=transforms.ToTensor(),
        )
    )
    dataset_test = (
        torchvision.datasets.MNIST(
            root=source, train=False, download=True,
            transform=transforms.ToTensor(),
        )
    )
    ...
    if optim_alg == "sgd":
        ...
        # optimizer = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=0, momentum = 0.9,nesterov=False)
    ...
    acc_test = evaluate(model, metric, minibatcher_test, device=device)
    ce_train = evaluate(model,loss,minibatcher_train,device=device)
    # ce_test = evaluate(model,loss,minibatcher_test,device=device)
