...
def subspace(size: int, /, *, n_cpus: int):
    ...
    onp.set_printoptions(suppress=True, precision=6)
    x1 = onp.arange(n).astype(float)
    x2 = rotate(x1.reshape(size, size), 1).reshape(n)
    print(onp.sum(x1 * eigenvectors[0]))
    print(onp.sum(x2 * eigenvectors[0]))
    exit()
